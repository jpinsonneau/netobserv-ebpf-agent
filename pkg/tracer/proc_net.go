...
func pidListensOnPort(pid int, port uint16) bool {
	if pid <= 0 || port == 0 {
		return false
	}
	conns, err := readProcTCPConns(pid)
	...
	for _, c := range conns {
		if c.localPort == port && c.state == 0x0a { // TCP_LISTEN
			return true
		}
	}
	return false
...
func pidListensOnFilterPorts(pid int, ports map[uint16]struct{}) bool {
	if pid <= 0 || len(ports) == 0 {
		return false
	}
	for port := range ports {
		if pidListensOnPort(pid, port) {
			return true
		}
	}
	return false
