...
type PacketFetcher struct {
	...
	plaintextScope           *PlaintextScope
	tlsCgroupAttach          *ktlsCgroupAttacher
...
func NewPacketFetcher(cfg *FlowFetcherConfig) (*PacketFetcher, error) {
	...
	if cfg.EnableKTLSTracking && newObjects.EnableKtlsTracking != nil {
		var enabled uint8
		if err := newObjects.EnableKtlsTracking.Get(&enabled); err != nil {
			plog.WithError(err).Warn("reading enable_ktls_tracking bpf variable")
		} else {
			plog.WithField("enable_ktls_tracking", enabled).Info("kTLS bpf variable")
		}
	}
	...
	if tlsSetup != nil {
		...
		pf.gotlsAttacher = tlsSetup.gotlsAttacher
		pf.tlsCgroupAttach = tlsSetup.ktlsCgroupAttach
		pf.ktlsStatsStop = tlsSetup.ktlsStatsStop
	}
...
func configureFlowSpecVariables(spec *cilium.CollectionSpec, cfg *FlowFetcherConfig, filter *Filter) error {
	...
	variables := []variablesMapping{
		...
		{constTraceMessages, traceMsgs},
		{constEnableRtt, enableRtt},
		{constEnableDNSTracking, enableDNSTracking},
		{constDNSTrackingPort, dnsTrackerPort},
		{constEnableFlowFiltering, enableFlowFiltering},
		{constEnableNetworkEventsMonitoring, enableNetworkEventsMonitoring},
		{constNetworkEventsMonitoringGroupID, uint8(networkEventsMonitoringGroupID)},
		{constEnablePktTranslation, enablePktTranslation},
		{constEnableIPsec, enableIPsec},
		{constEnableDirectFlowRingbuf, enableDirectFlowRingbuf},
		{constEnableOpenSSLTracking, enableOpenSSLTracking},
		{constEnableGoTLSTracking, enableGoTLSTracking},
		{constEnableKTLSTracking, enableKTLSTracking},
		{constEnableTLSUsageTracking, enableTLSTracking},
...
func configureFlowSpecMapSizing(spec *cilium.CollectionSpec, cfg *FlowFetcherConfig, filter *Filter) {
	shrinkMapWhenDisabled(spec, dnsLatencyMap, cfg.EnableDNSTracking)
	if filter == nil {
		spec.Maps[filterMap].MaxEntries = 1
		spec.Maps[peerFilterMap].MaxEntries = 1
	}
	shrinkMapWhenDisabled(spec, ipsecInputMap, cfg.EnableIPsecTracking)
	shrinkMapWhenDisabled(spec, ipsecOutputMap, cfg.EnableIPsecTracking)
}
